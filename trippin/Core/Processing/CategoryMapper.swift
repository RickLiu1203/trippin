...
enum CategoryMapper {
    ...
    private static let sceneryKeywords: Set<String> = [
        ...
        "sky", "sunset", "sunrise", "nature", "outdoor", "garden",
        "river", "waterfall", "snow", "cloud", "desert", "field",
        "meadow", "valley", "island", "coast", "cliff", "glacier",
        "volcano", "countryside", "rural", "panorama", "horizon",
        "wilderness", "rainbow", "fog", "aurora", "park",
    ]

    private static let landmarkKeywords: Set<String> = [
        "building", "architecture", "monument", "temple", "church",
        ...
        "mosque", "shrine", "memorial", "capitol", "library",
    ]
    ...
    static func map(observations: [VNClassificationObservation]) -> ClassificationResult {
        ...
        for observation in sorted {
            if let category = categoryFor(label: observation.identifier) {
                return ClassificationResult(category: category, confidence: Double(observation.confidence))
            }
        }
        return ClassificationResult(category: .activity, confidence: 0.5)
    }
}
